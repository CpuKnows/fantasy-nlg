...
class GenerateTemplates:
    ...
    def __init__(self, nlp, teams_file, vectorizer=None, clf=None):
        ...
        self.data_cols = [
            'player_name', 'player_position', 'team', 'week', 'game_dow', 'opp', 'away_game', 'team_score', 'opp_score',
    ...
    def doc_to_template(self, doc, inverted_news_dict, return_doc=False, training=True):
        ...
        if training:
            ...
            if return_doc:
                return Template(tagged_text), doc
            else:
    ...
    def create_training_data(self, news_stats_df, output_file):
        """
        ...
        for idx, doc, news_dict, inverted_news_dict in self.doc_preprocessing(news_stats_df):
            news_template, _, doc = self.doc_to_template(doc, inverted_news_dict, return_doc=True)
    ...
    def template_transformer(self, news_stats_df, output_file):
        """
        ...
        for idx, doc, news_dict, inverted_news_dict in self.doc_preprocessing(news_stats_df):
            news_template, doc = self.doc_to_template(doc, inverted_news_dict, return_doc=True, training=False)
